 ...
 *
 * Link : http://www.java2novice.com/java-sorting-algorithms/selection-sort/
 *
 * Selection sort is not difficult to analyze compared to other sorting algorithms since none of the loops depend on
 * the data in the array. Selecting the lowest element requires scanning all n elements (this takesn − 1 comparisons)
 * and then swapping it into the first position. Finding the next lowest element requires scanning the
 * remaining n − 1 elements and so on, for (n − 1) + (n − 2) + ... + 2 + 1 = n(n − 1) / 2 ∈ Θ(n2) comparisons.
